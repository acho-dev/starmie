...
class StarmieModelService:
    ...
    def __init__(self, datalake_path: str, results_dir: str = "./results"):
        ...
        # Cache for datalake vectors
        self.datalake_vectors = None
    ...
    def _load_datalake_vectors(self):
        """Load precomputed datalake vectors"""
        try:
            ...
            if vector_path.exists():
                ...
                for item in data:
                    filename = item[0]
                    vectors = item[1]  # This might be multiple column vectors for one table
                    
                    if isinstance(vectors, np.ndarray) and len(vectors.shape) == 2:
                        # Multiple column vectors - average them to get one vector per table
                        table_vector = np.mean(vectors, axis=0)
                        self.datalake_filenames.append(filename)
                        all_vectors.append(table_vector)
                    else:
                        # Single vector
                        self.datalake_filenames.append(filename)
                        all_vectors.append(vectors)
                
                self.datalake_vectors = np.vstack(all_vectors)
                
                logger.info(f"✅ Loaded datalake vectors: {len(self.datalake_filenames)} tables, {self.datalake_vectors.shape} vectors")
            else:
    ...
    def search_similar_tables(self, query_vectors: np.ndarray, top_k: int = 10, threshold: float = 0.7) -> List[Tuple[str, float]]:
        """Find similar tables using cosine similarity"""
        if self.datalake_vectors is None:
        ...
        try:
            ...
            logger.info(f"Datalake vectors shape: {self.datalake_vectors.shape}")
            logger.info(f"Number of datalake files: {len(self.datalake_filenames)}")
            
            # Handle case where query has multiple columns but we need to match against table-level vectors
            if len(query_vectors.shape) == 2 and query_vectors.shape[0] > 1:
                # Average the column vectors to get a single table representation
                query_vector = np.mean(query_vectors, axis=0, keepdims=True)
            else:
                query_vector = query_vectors.reshape(1, -1)
            
            logger.info(f"Processed query vector shape: {query_vector.shape}")
            
            # Normalize vectors
            query_norm = np.linalg.norm(query_vector, axis=1, keepdims=True)
            query_normalized = query_vector / (query_norm + 1e-8)
            
            datalake_norm = np.linalg.norm(self.datalake_vectors, axis=1, keepdims=True)
            datalake_normalized = self.datalake_vectors / (datalake_norm + 1e-8)
            
            # Compute cosine similarities
            similarities = (query_normalized @ datalake_normalized.T).flatten()
