...
if __name__ == '__main__':
    ...
    # Print match results for each query
    print(f"\nQuery: {q[0]}")
    print(f"Top {len(res)} matches:")
    for i, (score, table_id) in enumerate(res):
        print(f"  {i+1}. Table {table_id} (score: {score:.4f})")
    ...
    if hp.benchmark == 'santosLarge' or hp.benchmark == 'wdc' or hp.benchmark == 'demo':
