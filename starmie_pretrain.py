...
class StarmiePretrain:
    def __init__(self, datalake_path, cache_dir=None, results_dir=None):
        ...
        # Check if we're pointing to an existing dataset structure
        parent_dir = self.datalake_path.parent
        
        # Check if this looks like an existing dataset structure
        # (has datalake folder and is under data/ or has typical dataset folders)
    ...
    def setup_directories(self):
        ...
        self.datalake_dir = self.datalake_path if self.datalake_path.name == 'datalake' else self.data_dir / "datalake"
    ...
    def prepare_datalake(self):
        ...
        if self.data_dir == self.datalake_path.parent and self.datalake_path.name == 'datalake':
        ...
        # Clear existing directories only if we're creating new structure
        for dir_path in [self.tables_dir, self.datalake_dir]:
            if dir_path.exists() and dir_path != self.datalake_path:
        ...
        for csv_file in csv_files:
            for dest_dir in [self.tables_dir, self.datalake_dir]:
                if dest_dir == self.datalake_path:
